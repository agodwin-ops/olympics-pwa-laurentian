...
CREATE TABLE player_stats (
    ...
    gameboard_moves INTEGER DEFAULT 3,  -- Updated to start with 3 moves
    gold INTEGER DEFAULT 3,             -- Updated to start with 3 gold
...
CREATE TABLE player_skills (
    ...
    speed INTEGER DEFAULT 1 CHECK (speed >= 1 AND speed <= 5), -- Updated from 'jumping'
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE INDEX idx_lectures_unit_id ON lectures(unit_id);
...
1. REMOVED DANGEROUS LEADERBOARD VIEW - The original schema had a leaderboard view that exposed ALL student data to any authenticated user.

2. FIXED OVERLY PERMISSIVE GRANTS - Original schema granted "ALL ON ALL TABLES" to authenticated users. Now using minimal necessary permissions.

3. ADDED STRICT RLS POLICIES - Every table now has proper RLS policies ensuring users can only access their own data.

4. SECURED ADMIN-ONLY TABLES - admin_activity table is now restricted to admins only.

5. PROTECTED SENSITIVE DATA - All user-specific data (stats, skills, inventory, xp_entries, medals, dice_rolls) is now restricted to the owner only.

6. MAINTAINED EDUCATIONAL CONTENT ACCESS - Units, assignments, and gameboard stations remain accessible to all authenticated users as they are educational content.

7. UPDATED DEFAULTS - Updated gold and moves to start with 3 as requested.

STUDENTS CAN NOW ONLY:
- Read their own profile, stats, skills, inventory, medals, xp history, and dice rolls
- Read educational content (units, assignments, stations, published lectures, public resources)
- Create their own dice rolls and access logs

STUDENTS CANNOT:
- View other students' data of any kind
- Access admin activity logs
- View unpublished lectures or private resources
