...
interface SimpleStudentUploadProps {
  onClose: () => void;
  onUploadComplete: (result: BatchUploadResult) => void;
}

export default function SimpleStudentUpload({ onClose, onUploadComplete }: SimpleStudentUploadProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Upload students one by one - create incomplete profiles
  ...
  const downloadTemplate = () => {
    const csvContent = 'email,password\nstudent1@university.ca,TempPass123!\nstudent2@university.ca,SecurePass456';
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'student_accounts_template.csv';
    link.click();
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold">Batch Student Account Creation</h2>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <ul className="text-sm text-blue-700 mt-1 list-disc list-inside">
                  <li>Upload a CSV file with student <strong>email</strong> and <strong>password</strong> only</li>
                  <li>Students will login with these credentials</li>
                  <li>On first login, students choose their <strong>username</strong>, <strong>program</strong>, and <strong>profile picture</strong></li>
                  <li>This preserves the onboarding experience while pre-setting login credentials</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            >
              ...
              <p className="text-sm text-gray-500">
                Format: email,password (no headers needed)
              </p>
            </div>
          </div>
          ...
          {showPreview && preview.length > 0 && (
            <div className="mb-6">
                  ...
                  <tbody>
                    {preview.map((row, index) => (
                      <tr key={index} className="border-t">
                        ...
                        <td className="px-4 py-2 border-r text-sm font-mono">{'•'.repeat(row.password?.length || 0)}</td>
                        <td className="px-4 py-2 border-r text-sm text-gray-400 italic">Will choose on first login</td>
                        <td className="px-4 py-2 text-sm text-gray-400 italic">Will choose on first login</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <p className="text-sm text-gray-500 mt-2">
                Total accounts to create: {preview.length}
              </p>
            </div>
          )}
