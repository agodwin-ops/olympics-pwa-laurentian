-- Olympics PWA Database Schema - COMPLETE API COMPATIBLE VERSION
-- Every field the API actually uses, with correct data types
...
-- 4. Player Stats table with ALL fields the API uses
...
-- 5. XP Entries table with ALL fields the API uses
...
-- 8. Gameboard Stations table (INTEGER id to match API)
CREATE TABLE IF NOT EXISTS gameboard_stations (
    ...
    completion_reward_xp INTEGER DEFAULT 10,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Insert gameboard stations with exact IDs and fields API expects
