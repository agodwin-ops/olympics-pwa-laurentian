-- Olympics PWA Database Schema - CORRECT SKILLS VERSION
-- Uses actual 5 skills: strength, endurance, tactics, climbing, speed
...
-- Add compatible columns to gameboard_stations (respecting existing structure)
...
-- Player Inventory table for special items from Calgary Olympic venues
CREATE TABLE IF NOT EXISTS player_inventory (
    ...
    properties JSONB,
    acquired_from VARCHAR(200), -- Which Calgary Olympic venue gave this item
...
-- Gameboard Moves Log table - tracks every move with Calgary Olympic venue visits
CREATE TABLE IF NOT EXISTS gameboard_moves_log (
    ...
    skill_increases JSONB, -- Skills improved from the 5: strength, endurance, tactics, climbing, speed
    move_description TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Skill Increases Log table - tracks progression in the 5 actual skills
CREATE TABLE IF NOT EXISTS skill_increases_log (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    skill_name VARCHAR(100) NOT NULL, -- strength, endurance, tactics, climbing, or speed
    old_level INTEGER NOT NULL,
    new_level INTEGER NOT NULL,
    increase_amount INTEGER NOT NULL,
    source VARCHAR(100) NOT NULL, -- station_visit, dice_roll, quest_complete, admin_award
    source_details TEXT, -- Which Calgary Olympic venue, assignment, etc.
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Battleboard Encounters table - special events at Calgary Olympic venues
...
-- Initialize player skills for existing users with CORRECT 5 skills
INSERT INTO player_skills (user_id, strength, endurance, tactics, climbing, speed)
SELECT u.id, 1, 1, 1, 1, 1
FROM users u
